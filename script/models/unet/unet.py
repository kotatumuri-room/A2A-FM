...
class UNetModel(nn.Module):
    ...
    def __init__(
                ...
                if legacy:
                    ...
                    self.label_emb = nn.ModuleList(
                        [
                            ...
                            for _ in range(continous_condition)
                        ]
                    )  # MODIFYED PART
            else:
        ...
        self.out = nn.Sequential(
            ...
            nn.SiLU(),
            conv_nd(
                dims, input_ch, out_channels, 3, padding=1
            ),  # deleted the zero module
        )
    ...
    def forward(self, t, x, y=None):
        ...
        if self.num_classes is not None:
            assert y.shape[0] == x.shape[0], f"y:{y.shape}, x{x.shape}"
            y = y.reshape(x.shape[0], -1)
            for i in range(y.shape[1]):
                if self.legacy:
                    emb = emb + self.label_emb(y[:, i])
                else:
                    emb = emb + self.label_emb[i](y[:, i])
...
class UNetModelWrapper(UNetModel):
    ...
    def __init__(
        ...
        return super().__init__(
            ...
            use_new_attention_order=use_new_attention_order,
            continous_condition=continous_condition,
            legacy=legacy,
        )
